...
description: Manage a human-readable Markdown knowledge base across projects, including project-knowledge synthesis, local static-HTML browsing, portable backup, verification, restore, and cross-machine migration. Use when the user asks to locate, set up, capture, organize, synthesize, promote, link, rename, search, audit, browse as static HTML, back up, verify, restore, or migrate durable personal knowledge. Do not use for ordinary project documentation that is meant to stay only in the current project.
---
...
Initialize, Capture, Promote, Project Synthesis that writes, Link/move/rename, migration, deletion, and repair are write modes. Before detailed source reading, drafting, staging, or mutation, a primary agent must read [references/delegation.md](references/delegation.md) and spawn one isolated designated knowledge-base editor when subagent orchestration is available. This requirement is an explicit skill instruction to delegate, not an optional optimization.
...
- **Search** for questions about existing knowledge. This mode is read-only. Read [references/workflows.md](references/workflows.md#search) and [references/python-tools.md](references/python-tools.md).
...
- Ground all factual content strictly in authorized materials. Do not extrapolate ungrounded facts, private APIs, parameters, or speculative specifics from pre-trained model memory. Legitimate supplementary context drawn from within the knowledge base or authorized project must be explicitly labeled with its source (e.g. `> 补充来源: ...`).
...
- Keep the homepage focused on stable projects and human-maintained topic maps, with optional secondary type-browsing links. Maintain explicit collection regions as described in [reading navigation](references/navigation.md); each new formal entry needs one appropriate collection inbound link and its own source provenance. Do not automatically aggregate leaf entries or add reciprocal, sibling, or extra-parent links.
...
- Record concise usage feedback only upon observing concrete operational defects, following [references/usage-feedback.md](references/usage-feedback.md). Do not perform speculative evaluations, extra background scans, or unrequested repairs.

## Search and edits

Use `scripts/kb.py search` and `scripts/kb.py read` for heading-aware search and bounded section reading (see [references/python-tools.md](references/python-tools.md)). Use `rg` as an auxiliary search tool for regex search across text or when Python tools are unavailable. Exclude `content/archive/` from ordinary searches unless the user asks for historical or deprecated material.
